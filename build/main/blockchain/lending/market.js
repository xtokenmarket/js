...
const getLendingMarkets = async (address, provider) => {
    try {
        ...
        const [wbtcLendingCollateral, wethLendingCollateral, linkLendingCollateral,] = await Promise.all([
        ...
        const [wbtcLendingCollateralCap, wethLendingCollateralCap, linkLendingCollateralCap,] = await Promise.all([
        ...
        const [wbtcLendingCollateralDeposited, wethLendingCollateralDeposited, linkLendingCollateralDeposited,] = await Promise.all([
            erc20_1.getTokenBalance(abis_1.WBTC, abis_1.ADDRESSES[abis_1.LENDING_WBTC_MARKET][chainId], provider),
            erc20_1.getTokenBalance(abis_1.WETH, abis_1.ADDRESSES[abis_1.LENDING_WETH_MARKET][chainId], provider),
            erc20_1.getTokenBalance(abis_1.LINK, abis_1.ADDRESSES[abis_1.LENDING_LINK_MARKET][chainId], provider),
        ]);
        const [wbtcBorrowingLimit, wethBorrowingLimit, linkBorrowingLimit,] = await Promise.all([
        ...
        const [wbtcAllowance, wethAllowance, linkAllowance] = await Promise.all([
            erc20_1.getTokenAllowance(abis_1.WBTC, address, abis_1.ADDRESSES[abis_1.LENDING_WBTC_MARKET][chainId], provider),
            erc20_1.getTokenAllowance(abis_1.WETH, address, abis_1.ADDRESSES[abis_1.LENDING_WETH_MARKET][chainId], provider),
            erc20_1.getTokenAllowance(abis_1.LINK, address, abis_1.ADDRESSES[abis_1.LENDING_LINK_MARKET][chainId], provider),
        ]);
        return [
            {
                asset: abis_1.WBTC,
...
const supplyCollateral = async (marketName, amount, provider) => {
    ...
    if (approvedAmount.lt(amount)) {
        return Promise.reject(new Error('Please approve the tokens before adding collateral'));
    }
...
const _getApprovedAmount = async (marketName, address, provider) => {
    ...
    if (!tokenContract) {
        return Promise.reject(new Error(constants_1.Errors.CONTRACT_INITIALIZATION_FAILED));
    }
    return tokenContract.allowance(address, abis_1.ADDRESSES[marketName][network.chainId]);
};
