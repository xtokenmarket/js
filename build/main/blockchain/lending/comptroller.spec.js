...
ava_1.default('Check borrowing capacity for unowned address', async (t) => {
    const borrowingCapacity = await comptroller_1.getBorrowingCapacity(constants_spec_1.oneAddress, constants_spec_1.provider);
    console.log('[Lending] Borrowing capacity for unowned address:', borrowingCapacity);
    t.true(Number(borrowingCapacity) === 0);
});
ava_1.default('Check health ratio for unowned address', async (t) => {
    const healthRatio = await comptroller_1.getHealthRatio(constants_spec_1.oneAddress, constants_spec_1.provider);
