...
const approveUsdc = async (amount, provider) => {
    ...
    return usdcContract.approve(abis_1.ADDRESSES[abis_1.LENDING_LIQUIDITY_POOL][network.chainId], amount);
};
...
const borrowLiquidity = async (amount, provider) => {
    const liquidityPoolContract = await helper_1.getLiquidityPoolContract(provider);
...
const _getApprovedAmountUSDC = async (address, provider) => {
    ...
    return usdcContract.allowance(address, abis_1.ADDRESSES[abis_1.LENDING_LIQUIDITY_POOL][network.chainId]);
};
