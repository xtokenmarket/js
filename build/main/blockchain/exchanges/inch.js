...
const getInchEstimatedQuantity = async (tokenIn, symbol, amount, tradeType, provider) => {
    let inputAmount = parseEther(amount);
    const { inchLiquidityProtocolContract, network } = await helper_1.getXInchContracts(symbol, provider);
    const { chainId } = network;
    // Addresses
    const inchAddress = abis_1.ADDRESSES[abis_1.INCH][chainId];
    const xinchAddress = abis_1.ADDRESSES[symbol][chainId];
    // Contracts
    const inchPoolContract = utils_1.getInchPoolContract(symbol, provider, chainId);
    let expectedQty;
    // Get equivalent `ETH` quantity from the input `1INCH` amount
    if (tradeType === abis_1.BUY && tokenIn !== abis_1.ETH) {
        inputAmount = await helper_1.getExpectedRateInch(inchLiquidityProtocolContract, inchAddress, constants_1.AddressZero, inputAmount);
    }
    expectedQty = await inchPoolContract.getReturn(tradeType === abis_1.BUY ? constants_1.AddressZero : xinchAddress, tradeType === abis_1.BUY ? xinchAddress : constants_1.AddressZero, inputAmount);
    // Get final `1INCH` quantity from the estimated `ETH` quantity
    if (tradeType === abis_1.SELL && tokenIn !== abis_1.ETH) {
        expectedQty = await helper_1.getExpectedRateInch(inchLiquidityProtocolContract, constants_1.AddressZero, inchAddress, expectedQty);
    }
...
const getInchPortfolioItem = async (symbol, address, provider) => {
    ...
    const inchPoolBalances = await helper_2.getBalances(symbol, inchPoolAddress, priceUsd, provider, chainId);
