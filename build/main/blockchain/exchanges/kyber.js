...
const getKyberPortfolioItem = async (symbol, address, provider) => {
    const { kncContract, kyberProxyContract, network, xkncContract, } = await helper_1.getXKncContracts(symbol, provider);
    const { chainId } = network;
    // Addresses
    ...
    const { priceUsd } = await xknc_1.getXKncPrices(xkncContract, kncContract, kyberProxyContract);
