...
const getEthUsdcPrice = async (provider) => {
    ...
    const usdcAddress = abis_1.ADDRESSES[abis_1.USDC][chainId];
    const usdcToken = new sdk_1.Token(sdk_1.ChainId.MAINNET, usdcAddress, 6);
    const pair = await sdk_1.Fetcher.fetchPairData(usdcToken, sdk_1.WETH[usdcToken.chainId], provider);
...
const getEthTokenPrice = async (tokenAddress, isPriceInvert, provider) => {
    ...
    const pair = await sdk_1.Fetcher.fetchPairData(token, sdk_1.WETH[token.chainId], provider);
...
const getUniswapEstimatedQuantity = async (tokenIn, symbol, amount, tradeType, provider) => {
    const network = await provider.getNetwork();
    const { chainId } = network;
    ...
    const ethXKncPair = await sdk_1.Fetcher.fetchPairData(sdk_1.WETH[sdk_1.ChainId.MAINNET], xKNCToken, provider);
    const kncEthPair = await sdk_1.Fetcher.fetchPairData(kncToken, sdk_1.WETH[sdk_1.ChainId.MAINNET], provider);
    let pairs = [ethXKncPair];
    if (tokenIn !== abis_1.ETH) {
        pairs =
            tradeType === abis_1.BUY ? [kncEthPair, ethXKncPair] : [ethXKncPair, kncEthPair];
    }
...
const getUniswapPortfolioItem = async (symbol, address, provider) => {
    ...
    // Contracts
    const uniswapPoolContract = utils_1.getUniswapPoolContract(symbol, provider, chainId);
    ...
    const uniswapPoolBalances = await helper_2.getBalances(symbol, uniswapPoolAddress, priceUsd, provider, chainId, undefined, true);
