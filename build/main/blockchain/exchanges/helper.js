...
const getBalances = async (symbol, poolAddress, tokenPrice, provider, chainId, underlyingPrice, isWeth, xTokenBalance = bignumber_1.BigNumber.from('0'), ethBalance = bignumber_1.BigNumber.from('0')) => {
    // Addresses
    ...
    if (ethBalance.isZero()) {
        if (isWeth) {
            ...
            const wethContract = new ethers_1.ethers.Contract(wethAddress, abis_1.Abi.ERC20, provider);
    ...
    const ethVal = ethBalance.mul(utils_1.parseEther(ethUsdcPrice)).div(constants_1.DEC_18);
    let underlying;
    ...
    if (underlyingPrice) {
        ...
        const underlyingContract = new ethers_1.ethers.Contract(underlyingAddress, abis_1.Abi.ERC20, provider);
        const underlyingBalance = await underlyingContract.balanceOf(poolAddress);
        underlyingVal = underlyingBalance.mul(underlyingPrice).div(constants_1.DEC_18);
        underlying = {
