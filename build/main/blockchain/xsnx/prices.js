...
const getXSnxPrices = async (xsnxContract) => {
    try {
        ...
        const { snxContract, tradeAccountingContract } = await helper_1.getXSnxContracts(provider);
        const exchangeRatesContract = (await utils_3.getExchangeRateContract(provider));
        const [{ rate: snxUsdPrice }, { rate: ethUsdPrice }, contractDebtValue,] = await Promise.all([
            ...
            exchangeRatesContract.rateAndUpdatedTime(utils_1.formatBytes32String('sETH')),
        ...
        const priceUsd = issueTokenPriceInEth.mul(ethUsdPrice).div(constants_1.DEC_18);
        const sellPriceUsd = redeemTokenPriceEth.mul(ethUsdPrice).div(constants_1.DEC_18);
        ...
        return {
            ...
            sellPriceEth: utils_2.formatNumber(utils_1.formatEther(redeemTokenPriceEth)),
