...
const burnXAssetCLR = async (symbol, amount, provider) => {
    ...
    // Estimate `gasLimit`
    const gasLimit = utils_1.getPercentage(await xAssetCLRContract.estimateGas.burn(amount), constants_1.GAS_LIMIT_PERCENTAGE_DEFAULT);
...
const getExpectedQuantityOnBurnXAssetCLR = async (symbol, amount, provider) => {
    ...
    const { amount0, amount1 } = await xAssetCLRContract.getAmountsForLiquidity(proRataBalance);
    return {
