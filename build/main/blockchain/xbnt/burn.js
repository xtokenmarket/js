...
const burnXBnt = async (symbol, sellForEth, amount, provider) => {
    ...
    // Estimate `gasLimit`
    const gasLimit = utils_1.getPercentage(await xbntContract.estimateGas.burn(amount, sellForEth, '1'), sellForEth ? constants_1.GAS_LIMIT_PERCENTAGE_ETH : constants_1.GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xbntContract.burn(amount, sellForEth, '1', {
        gasLimit,
    });
};
exports.burnXBnt = burnXBnt;
const getExpectedQuantityOnBurnXBnt = async (symbol, sellForEth, amount, provider) => {
    ...
    const { BURN_FEE, proRataBnt } = await getProRataBnt(xbntContract, inputAmount);
    let expectedQty;
    if (!sellForEth) {
        ...
        const expectedRate = await utils_2.getExpectedRate(kyberProxyContract, bntAddress, ethAddress, proRataBnt);
