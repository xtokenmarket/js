...
ava_1.default('Get xINCHa prices', async (t) => {
    const { xinchContract } = await helper_1.getXInchContracts(abis_1.X_INCH_A, constants_spec_1.provider);
...
ava_1.default('Get xINCHb prices', async (t) => {
    const { xinchContract } = await helper_1.getXInchContracts(abis_1.X_INCH_B, constants_spec_1.provider);
    const { aum, priceEth, priceUsd } = await prices_1.getXInchPrices(xinchContract);
