...
const burnXKnc = async (symbol, sellForEth, amount, provider) => {
    ...
    const minRate = await utils_2.getExpectedRate(kyberProxyContract, tokenContract.address, abis_1.ADDRESSES[abis_1.ETH], amount, true);
    // Estimate `gasLimit`
    const gasLimit = utils_1.getPercentage(await xkncContract.estimateGas.burn(amount, !sellForEth, minRate), sellForEth ? constants_1.GAS_LIMIT_PERCENTAGE_ETH : constants_1.GAS_LIMIT_PERCENTAGE_DEFAULT);
...
const getExpectedQuantityOnBurnXKnc = async (symbol, sellForEth, amount, provider) => {
    ...
    if (!sellForEth) {
        ...
        const expectedRate = await utils_2.getExpectedRate(kyberProxyContract, kncAddress, ethAddress, proRataKnc);
