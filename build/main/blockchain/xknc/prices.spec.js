...
ava_1.default('Get xKNCa prices', async (t) => {
    const { xkncContract } = await helper_1.getXKncContracts(abis_1.X_KNC_A, constants_spec_1.provider);
...
ava_1.default('Get xKNCb prices', async (t) => {
    const { xkncContract } = await helper_1.getXKncContracts(abis_1.X_KNC_B, constants_spec_1.provider);
    const { aum, priceEth, priceUsd } = await prices_1.getXKncPrices(xkncContract);
