...
const burnXAave = async (symbol, sellForEth, amount, provider) => {
    ...
    const minRate = await utils_2.getExpectedRate(kyberProxyContract, tokenContract.address, abis_1.ADDRESSES[abis_1.ETH], proRataAave, true);
    // Estimate `gasLimit`
    const gasLimit = utils_1.getPercentage(await xaaveContract.estimateGas.burn(amount, sellForEth, minRate), sellForEth ? constants_1.GAS_LIMIT_PERCENTAGE_ETH : constants_1.GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xaaveContract.burn(amount, sellForEth, minRate, {
        gasLimit,
    });
};
exports.burnXAave = burnXAave;
const getExpectedQuantityOnBurnXAave = async (symbol, sellForEth, amount, provider) => {
    const inputAmount = parseEther(amount);
    const { kyberProxyContract, network, xaaveContract, } = await helper_1.getXAaveContracts(symbol, provider);
    const { chainId } = network;
    const { BURN_FEE, proRataAave } = await getProRataAave(xaaveContract, inputAmount);
    let expectedQty;
    if (!sellForEth) {
        ...
        const expectedRate = await utils_2.getExpectedRate(kyberProxyContract, aaveAddress, ethAddress, proRataAave);
