...
ava_1.default('Get xAAVEa prices', async (t) => {
    const { xaaveContract } = await helper_1.getXAaveContracts(abis_1.X_AAVE_A, constants_spec_1.provider);
    const { aum, priceEth, priceUsd } = await prices_1.getXAavePrices(xaaveContract);
...
ava_1.default('Get xAAVEb prices', async (t) => {
    const { xaaveContract } = await helper_1.getXAaveContracts(abis_1.X_AAVE_B, constants_spec_1.provider);
    const { aum, priceEth, priceUsd } = await prices_1.getXAavePrices(xaaveContract);
