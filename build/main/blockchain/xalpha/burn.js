...
const burnXAlpha = async (symbol, sellForEth, amount, provider) => {
    ...
    // estimate gasLimit
    const gasLimit = utils_1.getPercentage(await xalphaContract.estimateGas.burn(amount, sellForEth, '1'), sellForEth ? constants_1.GAS_LIMIT_PERCENTAGE_ETH : constants_1.GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xalphaContract.burn(amount, sellForEth, '0', {
        gasLimit,
    });
};
exports.burnXAlpha = burnXAlpha;
const getExpectedQuantityOnBurnXAlpha = async (symbol, sellForEth, amount, provider) => {
    const inputAmount = parseEther(amount);
    const { xalphaContract } = await helper_1.getXAlphaContracts(symbol, provider);
    const { BURN_FEE, proRataAlpha } = await getProRataAlpha(xalphaContract, inputAmount);
    let expectedQty;
    if (!sellForEth) {
        expectedQty = proRataAlpha;
    }
    else {
        expectedQty = parseEther(await uniswapV3_1.getUniswapV3EstimatedQty(abis_1.ETH, abis_1.X_ALPHA_A, formatEther(proRataAlpha), abis_1.SELL, bignumber_1.BigNumber.from('10000'), // 1% Uniswap V3 trade fees
        provider));
    }
