...
ava_1.default('Burn throws exceeded maximum redeemable error for huge amount', async (t) => {
    try {
...
ava_1.default('Expected quantity on burn throws error for invalid amount', async (t) => {
    try {
...
ava_1.default('Expected quantity on mint throws error for invalid amount', async (t) => {
    try {
...
ava_1.default('Best return on mint xAAVEa', async (t) => {
    const bestReturn = await xToken.getBestReturn(abis_1.X_AAVE_A, false, '1', abis_1.BUY);
...
ava_1.default('Best return on mint xALPHAa', async (t) => {
    const bestReturn = await xToken.getBestReturn(abis_1.X_ALPHA_A, false, '1', abis_1.BUY);
    console.log(JSON.stringify(bestReturn));
    t.true(Number(bestReturn.best.expectedQuantity) > 0);
});
