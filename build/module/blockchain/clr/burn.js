...
export const burnXAssetCLR = async (symbol, amount, provider) => {
    ...
    // Estimate `gasLimit`
    const gasLimit = getPercentage(await xAssetCLRContract.estimateGas.burn(amount), GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xAssetCLRContract.burn(amount, {
        gasLimit,
    });
};
export const getExpectedQuantityOnBurnXAssetCLR = async (symbol, amount, provider) => {
    const { xAssetCLRContract } = await getXAssetCLRContracts(symbol, provider);
