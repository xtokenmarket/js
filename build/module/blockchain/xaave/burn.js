...
export const burnXAave = async (symbol, sellForEth, amount, provider) => {
    ...
    const minRate = await getExpectedRate(kyberProxyContract, tokenContract.address, ADDRESSES[ETH], proRataAave, true);
    // Estimate `gasLimit`
    const gasLimit = getPercentage(await xaaveContract.estimateGas.burn(amount, sellForEth, minRate), sellForEth ? GAS_LIMIT_PERCENTAGE_ETH : GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xaaveContract.burn(amount, sellForEth, minRate, {
        gasLimit,
    });
};
export const getExpectedQuantityOnBurnXAave = async (symbol, sellForEth, amount, provider) => {
    ...
    if (!sellForEth) {
        ...
        const expectedRate = await getExpectedRate(kyberProxyContract, aaveAddress, ethAddress, proRataAave);
