...
export const burnXBnt = async (symbol, sellForEth, amount, provider) => {
    ...
    // Estimate `gasLimit`
    const gasLimit = getPercentage(await xbntContract.estimateGas.burn(amount, sellForEth, '1'), sellForEth ? GAS_LIMIT_PERCENTAGE_ETH : GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xbntContract.burn(amount, sellForEth, '1', {
        gasLimit,
    });
};
export const getExpectedQuantityOnBurnXBnt = async (symbol, sellForEth, amount, provider) => {
    ...
    if (!sellForEth) {
        ...
        const expectedRate = await getExpectedRate(kyberProxyContract, bntAddress, ethAddress, proRataBnt);
