...
export const burnXAlpha = async (symbol, sellForEth, amount, provider) => {
    ...
    // estimate gasLimit
    const gasLimit = getPercentage(await xalphaContract.estimateGas.burn(amount, sellForEth, '1'), sellForEth ? GAS_LIMIT_PERCENTAGE_ETH : GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xalphaContract.burn(amount, sellForEth, '0', {
        gasLimit,
    });
};
export const getExpectedQuantityOnBurnXAlpha = async (symbol, sellForEth, amount, provider) => {
    ...
    if (!sellForEth) {
        ...
        expectedQty = parseEther(await getUniswapV3EstimatedQty(ETH, X_ALPHA_A, formatEther(proRataAlpha), SELL, BigNumber.from('10000'), // 1% Uniswap V3 trade fees
        provider));
    }
