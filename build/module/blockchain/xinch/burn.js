...
export const burnXInch = async (symbol, sellForEth, amount, provider) => {
    ...
    const minRate = await getExpectedRateInch(inchLiquidityProtocolContract, tokenContract.address, AddressZero, proRataInch, true);
    // Estimate `gasLimit`
    const gasLimit = getPercentage(await xinchContract.estimateGas.burn(amount, sellForEth, minRate), sellForEth ? GAS_LIMIT_PERCENTAGE_ETH : GAS_LIMIT_PERCENTAGE_DEFAULT);
    return xinchContract.burn(amount, sellForEth, minRate, { gasLimit });
};
export const getExpectedQuantityOnBurnXInch = async (symbol, sellForEth, amount, provider) => {
    ...
    if (!sellForEth) {
        ...
        expectedQty = await getExpectedRateInch(inchLiquidityProtocolContract, inchAddress, AddressZero, proRataInch);
    }
