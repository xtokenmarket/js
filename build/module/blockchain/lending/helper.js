import { LENDING_COMPTROLLER, LENDING_LINK_MARKET, LENDING_LINK_PRICE, LENDING_LIQUIDITY_POOL, LENDING_LPT, LENDING_WBTC_MARKET, LENDING_WBTC_PRICE, LENDING_WETH_MARKET, LENDING_WETH_PRICE, } from '@xtoken/abis';
...
export const getComptrollerContract = async (provider) => {
    ...
    const comptrollerContract = getContract(LENDING_COMPTROLLER, provider, network);
...
export const getLiquidityPoolContract = async (provider) => {
    const network = await provider.getNetwork();
    const liquidityPoolContract = getContract(LENDING_LIQUIDITY_POOL, provider, network);
