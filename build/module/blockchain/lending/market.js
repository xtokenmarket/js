...
import { ADDRESSES, LENDING_LINK_MARKET, LENDING_WBTC_MARKET, LENDING_WETH_MARKET, 
// LENDING_X_AAVE_A_MARKET,
// LENDING_X_AAVE_B_MARKET,
// LENDING_X_INCH_A_MARKET,
LINK, WBTC, WETH, } from '@xtoken/abis';
...
export const getLendingMarkets = async (address, provider) => {
    try {
        ...
        const [wbtcAllowance, wethAllowance, linkAllowance] = await Promise.all([
            getTokenAllowance(WBTC, address, ADDRESSES[LENDING_WBTC_MARKET][chainId], provider),
            getTokenAllowance(WETH, address, ADDRESSES[LENDING_WETH_MARKET][chainId], provider),
            getTokenAllowance(LINK, address, ADDRESSES[LENDING_LINK_MARKET][chainId], provider),
        ]);
        return [
            {
                asset: WBTC,
...
const _getApprovedAmount = async (marketName, address, provider) => {
    ...
    return tokenContract.allowance(address, ADDRESSES[marketName][network.chainId]);
};
