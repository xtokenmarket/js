...
import { getBalancerEstimatedQuantity, getBalancerPortfolioItem, } from './balancer';
...
test('Calculate expected quantity on burn of xAAVEb on Balancer', async (t) => {
    const expectedQty = await getBalancerEstimatedQuantity(X_AAVE_B, X_AAVE_B, '1000', SELL, provider);
...
test('Calculate expected quantity on mint of xAAVEb on Balancer', async (t) => {
    const expectedQty = await getBalancerEstimatedQuantity(ETH, X_AAVE_B, '1', BUY, provider);
...
test('Get Balancer Portfolio of xAAVEa', async (t) => {
    const portfolio = await getBalancerPortfolioItem(X_AAVE_A, testAddress, provider);
...
test('Get Balancer Portfolio of xAAVEb', async (t) => {
    const portfolio = await getBalancerPortfolioItem(X_AAVE_B, testAddress, provider);
