...
export const getInchEstimatedQuantity = async (tokenIn, symbol, amount, tradeType, provider) => {
    let inputAmount = parseEther(amount);
    const { inchLiquidityProtocolContract, network } = await getXInchContracts(symbol, provider);
    ...
    if (tradeType === BUY && tokenIn !== ETH) {
        inputAmount = await getExpectedRateInch(inchLiquidityProtocolContract, inchAddress, AddressZero, inputAmount);
    }
    expectedQty = await inchPoolContract.getReturn(tradeType === BUY ? AddressZero : xinchAddress, tradeType === BUY ? xinchAddress : AddressZero, inputAmount);
    // Get final `1INCH` quantity from the estimated `ETH` quantity
    if (tradeType === SELL && tokenIn !== ETH) {
        expectedQty = await getExpectedRateInch(inchLiquidityProtocolContract, AddressZero, inchAddress, expectedQty);
    }
...
export const getInchPortfolioItem = async (symbol, address, provider) => {
    ...
    const inchPoolBalances = await getBalances(symbol, inchPoolAddress, priceUsd, provider, chainId);
