...
export const getKyberPortfolioItem = async (symbol, address, provider) => {
    const { kncContract, kyberProxyContract, network, xkncContract, } = await getXKncContracts(symbol, provider);
    const { chainId } = network;
    // Addresses
    ...
    const { priceUsd } = await getXKncPrices(xkncContract, kncContract, kyberProxyContract);
